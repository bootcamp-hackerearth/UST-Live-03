...
public class RoleDaoImpl implements RoleDao {
    ...
    public Role findByIdentifier(String identifier) {
        ...
        List<Role> roleList = jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(Role.class), identifier);
        return roleList.isEmpty() ? null : roleList.get(0);
    }

    @Override
    public Role save(RoleDto roleDto) {
        String sql = "INSERT INTO ROLE (identifier, description, status, success) VALUES (?, ?, ?, ?)";
        jdbcTemplate.update(sql, roleDto.getIdentifier(), roleDto.getDescription(), roleDto.isStatus(), roleDto.isSuccess());
