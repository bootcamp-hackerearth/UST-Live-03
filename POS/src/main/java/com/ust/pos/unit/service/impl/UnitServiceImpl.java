...
public class UnitServiceImpl implements UnitService {
    ...
    public UnitDto findByIdentifier(String identifier) {
        Unit unit = unitRepository.findByIdentifier(identifier);
        if (unit == null) {
            throw new ResourceNotFoundException(UNIT_WITH_IDENTIFIER + identifier + " not found");
        }
    ...
    @Override
    public PaginatedResponseDto<UnitDto> findAll(Specification<Unit> example, Pageable pageable) {

        Type listType = new TypeToken<List<UnitDto>>() {
        }.getType();
        Page<Unit> page = unitRepository.findAll(example, pageable);

        PaginatedResponseDto<UnitDto> paginatedResponseDto = new PaginatedResponseDto<>();
