...
public class OrderServiceImpl extends BaseService implements OrderService {
    ...
    public OrderDto checkout(OrderDto orderDto) {
        ...
        List<CartEntry> cartEntryList = cartEntryRepository.findByCartId(cart.getIdentifier());
        ...
        for (CartEntry cartEntry : cartEntryList) {
            ...
            orderEntry.setIdentifier(orderIdentifier + "-" + cartEntry.getProductId());
            orderEntry.setOrderIdentifier(orderIdentifier);
            orderEntry.setProductIdentifier(cartEntry.getProductId());
            orderEntry.setMrp(cartEntry.getMrp());
            orderEntry.setUnitDiscount(cartEntry.getDiscount());
            orderEntry.setUnitPrice(cartEntry.getSellingPrice());
    ...
    public PaginationResponseDto<OrderDto> findAll(Pageable pageable) {
        ...
        PaginationResponseDto<OrderDto> orderPaginationDto = new PaginationResponseDto<>();
        orderPaginationDto.setDtoList(modelMapper.map(orderPage.getContent(), listType));
        orderPaginationDto.setTotalRecords(orderPage.getTotalElements());
        orderPaginationDto.setTotalPages(orderPage.getTotalPages());
        orderPaginationDto.setSizePerPage(pageable.getPageSize());
        orderPaginationDto.setPage(pageable.getPageNumber());
