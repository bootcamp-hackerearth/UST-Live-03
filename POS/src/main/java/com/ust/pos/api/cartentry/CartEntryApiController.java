...
public class CartEntryApiController extends BaseController {
    ...
    @DeleteMapping("/deleteAllByCart")
    public boolean delete(@RequestBody CartEntryDto cartEntryDto) {
        try {
            cartEntryService.deleteAllByCart(cartEntryDto.getCart());
    ...
    @PostMapping("/getEntriesList")
    public List<CartEntryDto> list2(@RequestParam String cart) {
        return cartEntryService.findAllCarts(cart);
    }

    @DeleteMapping("/deleteEntries")
    public boolean delete2(@RequestBody CartEntryDto cartEntryDto) {
        try {
            cartEntryService.delete(cartEntryDto.getProduct(), cartEntryDto.getCart());
            cartService.recalculate(cartEntryDto.getCart());
        } catch (Exception e) {
            return false;
        }
