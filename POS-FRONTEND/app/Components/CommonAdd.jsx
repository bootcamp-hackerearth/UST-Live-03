...
function CommonAdd({
...
})
 {
  ...
  const buildPayload = () => {
    const flatPayload = {
      ...(showIdentifierDescription ? { identifier } : {}),
      ...values,
      ...extraData,
    };

    extraFields.forEach((field) => {
      if (field.valueFormat === "csv" && Array.isArray(flatPayload[field.key])) {
        flatPayload[field.key] = flatPayload[field.key].join(",");
      }

      if (field.valueType === "boolean") {
        flatPayload[field.key] =
          flatPayload[field.key] === true || flatPayload[field.key] === "true";
      }

      if (
        field.type === "number" &&
        flatPayload[field.key] !== "" &&
        flatPayload[field.key] !== null &&
        flatPayload[field.key] !== undefined
      ) {
        const parsedNumber = Number(flatPayload[field.key]);
        if (Number.isFinite(parsedNumber)) {
          flatPayload[field.key] = parsedNumber;
        }
      }
    });
    ...
    Object.keys(flatPayload).forEach((key) => {
      if (key.includes(".")) {
        setNestedValue(nestedPayload, key, flatPayload[key]);
      } else {
        nestedPayload[key] = flatPayload[key];
      }
    });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      console.log("NESTED PAYLOAD SENDING TO BACKEND:", finalPayload);
      ...
      setError(
        status ? `${message} (HTTP ${status} on ${endpoint})` : message
      );
  ...
  return (
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-3xl mx-auto">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-black">Add {title}</h2>
