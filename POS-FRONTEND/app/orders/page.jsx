...
export default function OrdersPage() {
    ...
    /* ─── fetch a page of orders from the paginated endpoint ─── */
    ...
    const handleInvoicePrint = (order) => {
        ...
        const items = order.entryDtoList || [];
        ...
        const itemsHtml = items.length === 0
            ...
            : items.map(entry => {
                ...
                return `
                <div class="item-row">
                    <div class="item-name">${entry.product || entry.identifier || "Item"}</div>
                    <div class="item-meta">
    ...
    /* ─── compact page number list, mirrors Node listing (1,2,3 ... with prev/next) ─── */
    ...
    return (
        <div style={{ position: "fixed", top: "60px", right: 0, bottom: 0, left: isSidebarOpen ? "220px" : "55px", backgroundColor: "#f4f5f9", fontFamily: "'Segoe UI', sans-serif", display: "flex", flexDirection: "column", overflow: "hidden", transition: "left 0.2s ease" }}>
            ...
            {/* SUB-MODULE DATA TOP BAR HEADER */}
                                        ...
                                        <tbody>
                                            {(selectedOrder.entryDtoList || []).map((entry, idx) => (
                                                <tr key={entry.identifier || idx} style={{ borderBottom: "1px solid #f0f1f6", color: C.text }}>
                                                    <td style={{ padding: "10px 6px" }}><div style={{ fontWeight: "600" }}>{entry.product}</div><div style={{ fontSize: "11px", color: C.muted }}>ID: {entry.identifier}</div></td>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                <div style={{ background: "#fff", padding: "12px 20px", borderBottom: "1px solid #e8eaf0", display: "flex", gap: "10px", alignItems: "center", flexShrink: 0 }}>
                    ...
                    <button type="button" onClick={() => fetchOrders(page)} style={{ height: "38px", padding: "0 18px", background: C.mid, color: "#fff", border: "none", borderRadius: "7px", fontWeight: "600", fontSize: "13px", cursor: "pointer", whiteSpace: "nowrap" }}>🔄 Refresh</button>
                </div>
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
                    {ordersLoading ? (
                        <div style={{ padding: "60px", textAlign: "center", color: C.muted, fontSize: "14px" }}>Loading orders...</div>
                    ) : ordersError ? (
                        <div style={{ padding: "40px", textAlign: "center", color: C.red, fontSize: "14px" }}>{ordersError}</div>
                    ) : filteredOrders.length === 0 ? (
                        <div style={{ padding: "60px", textAlign: "center", color: C.muted, fontSize: "14px" }}>No orders found.</div>
                    ) : (
                ...
                {/* PAGINATION FOOTER — same pattern as Node listing page */}
                {!ordersLoading && !ordersError && totalPages > 1 && (
                    <div style={{
                        ...
                        <span style={{ fontSize: "13px", color: C.muted, marginRight: "8px" }}>
