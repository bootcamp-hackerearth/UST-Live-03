  ...
  useEffect(() => {
    const fetchInvoiceDetails = async () => {
      ...
      try {
        ...
        console.error("Error fetching invoice layout data profile:", err);
        alert("Failed to locate invoice entry record.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 text-sm text-slate-500 font-medium">
        Loading receipt configuration snapshots...
      </div>
    );
  }
  ...
  if (!order) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
        <p className="text-sm text-slate-500">Invoice statement could not be resolved.</p>
        <button onClick={() => router.push("/orders")} className="text-xs bg-slate-800 text-white px-4 py-2 rounded-lg">
          Back to List
        </button>
      </div>
    );
      ...
      <div className="max-w-md mx-auto bg-white border border-slate-200 p-6 shadow-sm rounded-xl print:border-none print:shadow-none font-mono text-xs">
        ...
        <div className="space-y-1 mb-4 text-[11px] text-slate-600">
          <div className="flex justify-between">
            <span>INVOICE :</span>
            <span className="font-bold text-slate-900">{order.invoiceCode}</span>
          </div>
          <div className="flex justify-between">
            <span>CUSTOMER:</span>
            <span className="font-bold text-slate-900">{order.customerIdentifier}</span>
          </div>
          <div className="flex justify-between">
            <span>DATE    :</span>
            <span>{order.createdOn ? new Date(order.createdOn).toLocaleString() : "N/A"}</span>
          </div>
        </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {order.entryList?.map((entry, index) => (
              <tr key={entry.identifier || index} className="text-slate-700">
                <td className="py-2 font-semibold truncate max-w-[140px]">{entry.product}</td>
                <td className="py-2 text-center">{entry.quantity}</td>
                <td className="py-2 text-right">${Number(entry.price || 0).toFixed(2)}</td>
                <td className="py-2 text-right font-bold">${Number(entry.totalPrice || 0).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="space-y-1.5 pt-1 text-[11px]">
          ...
          <div className="flex justify-between text-rose-600">
            <span>Total Campaign Discount:</span>
