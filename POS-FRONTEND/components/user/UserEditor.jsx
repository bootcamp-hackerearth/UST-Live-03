...
export default function UserEditor({
  ...
  const validateForm = () => {
    if (name.trim() === "" || username.trim() === "" || phoneNo.trim() === "") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {id,name,username,phoneNo,roles};
      const response = await updateItem("user",payload);
      if (!response.success) {
      ...
      if (refreshCurrentUserIfEditingSelf && user?.id === id) {
  ...
  if (pageLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <p className="text-[#667085]">Loading user...</p>
      </div>
    );
  }
  return (
