...
const DashboardLayout = ({ children }) => {
  ...
  const fetchNodes = async () => {
    try {
      ...
      localStorage.setItem("allowedNodes", JSON.stringify(allowedNodes));
    } catch (err) {
      console.log(err);
    }
  };
  ...
  useEffect(() => {
    ...
    const validateUserToken = async () => {
      ...
      if (!token || !username) { localStorage.clear(); router.push("/login"); return; }
      try {
        const res = await api.post("/validateToken", { token, username });
        if (res.data !== true) logoutDeletedUser();
      } catch (err) {
    ...
    validateUserToken();
    const interval = setInterval(validateUserToken, 5000);
    return () => clearInterval(interval);
  }, [router, accountDeleted]);

  const mainLeft = sidebarCollapsed ? "ml-[64px]" : "ml-[240px]";
  ...
  return (
    <div className="min-h-screen bg-[#E9EEF5]">
      ...
      <div className={`${mainLeft} pt-20 p-8 transition-all duration-300`}>
        {children}
      </div>
    </div>
  );
