...
export default function CommonEdit({
  ...
  const managedIdentifier = rawIdentifier
    ...
    const identifier = isManagedMode
      ? managedIdentifier
      : identifierProp;

  const description = isManagedMode ? managedDescription : descriptionProp;
  const setDescription = isManagedMode ? setManagedDescription : setDescriptionProp;
  const resolvedLoading = isManagedMode ? managedLoading : loadingProp;
  const resolvedError = isManagedMode ? managedError : errorProp;
  const resolvedSuccessMessage = isManagedMode ? managedSuccessMessage : successMessageProp;
  ...
  useEffect(() => {
    ...
    const loadEntity = async () => {
      try {
        ...
        console.log(err);
        if (entityType === "model" || entityType === "brand") {
          setManagedError(err?.response?.data?.message || `Failed to load ${entityType}`);
        } else {
          setManagedError(`Failed to load ${entityType}`);
        }
      } finally {
  ...
  const managedSubmit = async (e) => {
    ...
    try {
      ...
      console.log(err);
      setManagedError(err?.response?.data?.message || `Failed to update ${entityType}`);
    } finally {
  ...
  const showAudit = resolvedAuditData?.createdBy || resolvedAuditData?.createdOn;
        ...
        {showAudit && (
          <div className="mb-6 grid grid-cols-2 gap-x-8 gap-y-4 rounded-2xl border border-[#e4e7ec] bg-[#f9fafb] px-6 py-4">
            <div className="flex flex-col gap-4">
              <div>
                <p className="text-xs font-medium text-[#667085] uppercase tracking-wide">Created By</p>
                <p className="mt-1 text-sm text-[#101828]">{resolvedAuditData?.createdBy || "—"}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-[#667085] uppercase tracking-wide">Created On</p>
                <p className="mt-1 text-sm text-[#101828]">{formatDate(resolvedAuditData?.createdOn)}</p>
              </div>
            </div>
            <div className="flex flex-col gap-4">
              <div>
                <p className="text-xs font-medium text-[#667085] uppercase tracking-wide">Modified By</p>
                <p className="mt-1 text-sm text-[#101828]">{resolvedAuditData?.modifiedBy || "—"}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-[#667085] uppercase tracking-wide">Modified On</p>
                <p className="mt-1 text-sm text-[#101828]">{formatDate(resolvedAuditData?.modifiedOn)}</p>
              </div>
            </div>
          </div>
        )}
